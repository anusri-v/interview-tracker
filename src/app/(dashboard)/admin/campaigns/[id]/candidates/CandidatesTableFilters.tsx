...
const STATUS_OPTIONS: { value: DisplayStatus; label: string }[] = [
  ...
  { value: "offer_in_process", label: "Offer In Process" },
...
export default function CandidatesTableFilters({
  ...
  const [currentRole, setCurrentRole] = useState(roleFilter);
  useEffect(() => setCurrentRole(roleFilter), [roleFilter]);
  ...
  function updateFilters(newSearch: string, newStatuses: DisplayStatus[], newSort: SortOption, newRound?: string, newRole?: string) {
        ...
        <div className="relative">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              updateFilters(searchValue, selectedStatuses, sort, undefined, e.target.value);
            }}
            disabled={isPending}
            className="text-sm border border-border rounded-lg pl-9 pr-3 py-2.5 bg-card text-foreground appearance-none cursor-pointer"
          >
